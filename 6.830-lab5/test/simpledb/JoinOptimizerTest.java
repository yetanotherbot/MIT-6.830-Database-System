...
public class JoinOptimizerTest extends SimpleDbTestBase {
	...
	@Test public void estimateJoinCostTest() throws ParsingException {
		...
		jo = new JoinOptimizer(Parser.generateLogicalPlan(tid, "SELECT * FROM " + tableName1 + " t1, " + tableName2 + " t2 WHERE t1.c1 = t2.c2;"), 
		...
		checkJoinEstimateCosts(jo, equalsJoinNode);
		// 1 join 1
		jo = new JoinOptimizer(Parser.generateLogicalPlan(tid, "SELECT * FROM " + tableName1 + " t1, " + tableName1 + " t2 WHERE t1.c3 = t2.c4;"), 
				new Vector<LogicalJoinNode>());
		equalsJoinNode = new LogicalJoinNode(tableName1, tableName1, Integer.toString(3), Integer.toString(4), Predicate.Op.EQUALS);
		checkJoinEstimateCosts(jo, equalsJoinNode);	
		// 2 join 2
		jo = new JoinOptimizer(Parser.generateLogicalPlan(tid, "SELECT * FROM " + tableName2 + " t1, " + tableName2 + " t2 WHERE t1.c8 = t2.c7;"), 
				new Vector<LogicalJoinNode>());
	...
	@Test public void estimateJoinCardinality() throws ParsingException {
        TransactionId tid = new TransactionId();
		JoinOptimizer j = new JoinOptimizer(Parser.generateLogicalPlan(tid, "SELECT * FROM " + tableName2 + " t1, " + tableName2 + " t2 WHERE t1.c8 = t2.c7;"), 
		new Vector<LogicalJoinNode>());
	...
	@Test public void orderJoinsTest() throws ParsingException, IOException, DbException, TransactionAbortedException {
		...
		Assert.assertFalse(result.get(0).t1 == "hobbies");
	...
	@Test(timeout=60000) public void bigOrderJoinsTest() throws IOException, DbException, TransactionAbortedException, ParsingException {
		...
		nodes.add(new LogicalJoinNode("a", "b", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("b", "c", "c0", "c1", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("c", "d", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("d", "e", "c0", "c1", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("e", "f", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("f", "g", "c0", "c1", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("g", "h", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("h", "i", "c0", "c1", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("i", "j", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("j", "k", "c0", "c1", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("k", "l", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("l", "m", "c0", "c1", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("m", "n", "c1", "c0", Predicate.Op.EQUALS));
		nodes.add(new LogicalJoinNode("n", "bigTable", "c0", "c1", Predicate.Op.EQUALS));
		
		// Make sure we don't give the nodes to the optimizer in a nice order
		Collections.shuffle(nodes);
		
		j = new JoinOptimizer(
				Parser.generateLogicalPlan(tid, "SELECT COUNT(a.c0) FROM bigTable, a, b, c, d, e, f, g, h, i, j, k, l, m, n WHERE bigTable.c0 = n.c0 AND a.c1 = b.c1 AND b.c0 = c.c0 AND c.c1 = d.c1 AND d.c0 = e.c0 AND e.c1 = f.c1 AND f.c0 = g.c0 AND g.c1 = h.c1 AND h.c0 = i.c0 AND i.c1 = j.c1 AND j.c0 = k.c0 AND k.c1 = l.c1 AND l.c0 = m.c0 AND m.c1 = n.c1;"),
				nodes);
		...
		Assert.assertEquals(result.get(result.size()-1).t2, "bigTable");		
	}
